...
题目描述：
		In a row of seats, 1 represents a person sitting in that
seat, and 0 represents that the seat is empty.
...
Example 1:
	Input: [1,0,0,0,1,0,1]
	Output:  2
	Explanation:If Alex sits in the second open seat (seats[2]),
...
Example 2:
	Input: [1,0,0,0]
	Output:  3
	Explanation:If Alex sits in the last seat ,the closest
person is 3 seats away.
	This is the maximum distance possible, so the answer is 3.
Note:
	1. 1 <= seats.length <= 20000
	2. seats contains only 0s or 1s, at least one 0,
	   and at least one 1.
"""
class Solution:
        ...
        for i in range(len(seats)):
            if seats[i] == 1:
                if initia < 0:
                    ...
                    result = max(result , (i - initia)//2)
                initia = i
        return max(result , len(seats) - 1 - initia)
